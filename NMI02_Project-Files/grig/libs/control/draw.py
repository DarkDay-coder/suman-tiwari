...
class Draw(object):
    def __init__(self, curve=None):
        ...
        if curve:
            self.curve = gnCommon.get_transform(curve)
        elif len(cmds.ls(selection=True)):
            self.curve = gnCommon.get_transform(cmds.ls(selection=True)[0])
        else:
            self.curve = None
    ...
    def write_curve(self, control=None, name=None, force=False):
        ...
        if control:
            self.curve = gnCommon.get_transform(control)
        elif len(cmds.ls(selection=True)):
            sel = cmds.ls(selection=True)
            self.curve = gnCommon.get_transform(sel[0])
        elif self.curve:
            pass
        else:
    ...
    def get_curve_info(self, curve=None):
        ...
        for crv in gnCommon.get_shapes(curve):
    ...
    def create_curve(self, name='default', shape='circle', axis='y', scale=1):
        ...
        for i, shp in enumerate(curve_dict):
            ...
            info = curve_dict[shp]
            # if this is the first time through he loop, create our main curve
            point_info = []
            ...
            if i == 0:
                self.curve = cmds.curve(point=point_info,
                                        degree=info['degree'],
                                        name=name)
                crv_shape = gnCommon.get_shapes(self.curve)[0]
            ...
            # the main curve and delete the extra transform
                ...
                child_crv = cmds.curve(point=point_info,
                                       degree=info['degree'])
                crv_shape = gnCommon.get_shapes(child_crv)[0]
        ...
        curve_shapes = gnCommon.get_shapes(self.curve)
