...
import grig.libs.space as gnSpace
reload(gnCtrl)
reload(gnAttr)
...
def get_control_types():
    ...
    for x in cmds.ls('*.ctrlDict'):
        ctrl = gnCtrl.Control(ctrl=x.split('.')[0])
...
def add_dislpay_type(node, value, name, target):
    dt = gnAttr.Attribute(node=node, type='enum', value=value,
...
def add_vis_ctrl():
    ...
    v_ctrl = gnCtrl.Control(parent=par,
                            shape='v',
                            prefix=None,
                            suffix='CTRL',
                            name='visibility',
    ...
    model_vis = gnAttr.Attribute(node=v_ctrl.ctrl, type='bool', value=1,
                                 keyable=True, name='modelVis')
    skel_vis = gnAttr.Attribute(node=v_ctrl.ctrl, type='bool', value=0,
                                keyable=True, name='skelVis')
    rig_vis = gnAttr.Attribute(node=v_ctrl.ctrl, type='bool', value=0,
                               keyable=True, name='rigVis')

    # add display type separator
    gnAttr.Attribute(node=v_ctrl.ctrl, type='separator', value=0,
                     name='displayType')
    ...
    for part, sides in part_dict.items():
        p_vis = gnAttr.Attribute(node=v_ctrl.ctrl, type='bool', value=1,
    ...
    for typ, ctrl_list in type_dict.items():
        t_vis = gnAttr.Attribute(node=v_ctrl.ctrl, type='bool', value=1,
...
def add_color_attributes():
    ...
    c_ctrl = gnCtrl.Control(parent=par,
                            shape='brush3',
                            prefix=None,
                            suffix='CTRL',
                            name='color',
    ...
    for typ, ctrl_list in type_dict.items():
        gnAttr.Attribute(node=c_ctrl.ctrl, type='separator', name=typ)
        clr = gnAttr.Attribute(node=c_ctrl.ctrl, type='double3', value=0,
                ...
                if cmds.objExists(attr_name):
                    ...
                    if cmds.objExists(part + '.' + driver):
                        ...
                        if all(cmds.objExists(ob) for ob in target_list[:-1]):
                            gnSpace.space_switch(node=driver.top,
                                                 driver=driver.ctrl,
        ...
        if cmds.objExists(part + '.transferAttributes'):
            ...
            for driven in driven_list[1:]:
                ...
                for attr in attr_list:
                    if attr != 'ctrlDict':
                        src_attr = gnAttr.Attribute(add=False,
                                                    node=driven,
                                                    name=attr,
...
def add_global_scale(global_ctrl='Cn_global_CTRL'):
    ...
    gs = gnAttr.Attribute(node=global_ctrl, type='double', value=1,
...
def add_switch_ctrl():
    ...
    s_ctrl = gnCtrl.Control(parent=par,
                            shape='switch',
                            prefix=None,
                            suffix='CTRL',
                            name='switch',
    ...
    for part in cmds.listRelatives('RIG'):
        if cmds.objExists(part + '.switchRigPlugs'):
            ...
            if cmds.objExists(s_ctrl.ctrl + '.' + switch_name):
                switch_attr = gnAttr.Attribute(node=s_ctrl.ctrl,
                                               name=switch_name,
                                               add=False)
            else:
                switch_attr = gnAttr.Attribute(node=s_ctrl.ctrl, type='double',
