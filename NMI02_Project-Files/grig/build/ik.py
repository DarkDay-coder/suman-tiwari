...
class Ik:
    ...
    def check_pv_guide(self):
        if self.pv_guide == 'auto':
            self.pv_guide = gnGuide.create_pv_guide(guide_list=self.guide_list,
                                                    name=self.base_name,
    ...
    def build_ik_controls(self):
        ...
        self.base_ctrl = gnCtrl.Control(parent=self.ik_ctrl_grp,
                                        shape='cube',
                                        prefix=self.side,
                                        suffix='CTRL',
        ...
        self.main_ctrl = gnCtrl.Control(parent=self.ik_ctrl_grp,
                                        shape='cube',
                                        prefix=self.side,
                                        suffix='CTRL',
        ...
        if self.pv_guide:
            self.pv_ctrl = gnCtrl.Control(parent=self.ik_ctrl_grp,
                                          shape='locator_3d',
                                          prefix=self.side,
                                          suffix='CTRL',
    ...
    def build_ik_chain(self):
        self.ik_chain = gnChain.Chain(transform_list=self.guide_list,
                                      prefix=self.side,
    ...
    def build_ikh(self, scale_attr=None, constrain=True):
        ...
        if self.pv_guide:
            ...
            gde = gnGuide.create_line_guide(a=self.pv_ctrl.ctrl,
                                            b=self.ik_joints[1],
                                            name=self.base_name)
        ...
        if self.stretchy:
            ...
            self.stretch_switch = gnAttr.Attribute(node=self.main_ctrl.ctrl,
                                                   type='double', value=1,
