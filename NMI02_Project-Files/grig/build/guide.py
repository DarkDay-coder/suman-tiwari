...
def create_pv_guide(guide_list=None,
    ...
    if slide_pv:
        ...
        a_len = gnMath.distance_between(point_a=guide_list[0],
                                        point_b=guide_list[1])
        b_len = gnMath.distance_between(point_a=guide_list[1],
                                        point_b=guide_list[2])
        total_len = a_len + b_len
        slide_ratio = float(b_len) / total_len

    offset = gnAttr.Attribute(node=pv_loc, type='double', value=offset_pv,
                              keyable=True, name='offset')
    slide = gnAttr.Attribute(node=pv_loc, type='double', min=0, max=1,
                             value=slide_ratio,
