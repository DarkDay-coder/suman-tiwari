...
import grig.build.spline as gnSpline
...
class Spine(gnModule.RigModule, gnSpline.Spline):
    def __init__(self,
                 side=None,
                 part=None,
                 guide_list=None,
                 ...
                 model_path=None,
                 guide_path=None):
        super(Spine, self).__init__(side=side, part=part,
    ...
    def create_module(self):
        super(Spine, self).create_module()
    ...
    def control_rig(self):
        ...
        # build fk controls
        fk_chain = gnChain.Chain(prefix=self.side,
                                 suffix='ctrl_JNT',
                                 name=self.part)
        ...
        for i, jnt in enumerate(fk_chain.joints[:-1]):
            ...
            if i == 0:
                ...
                shape = 'halfCylinder'
            fk_ctrl = gnCtrl.Control(parent=par,
                                     shape=shape,
                                     prefix=self.side,
                                     suffix='CTRL',
                                     name=ctrl_name,
    ...
    def output_rig(self):
        ...
        if self.mid_ctrl:
            blend = gnAttr.Attribute(node=self.mid_ctrl.ctrl, type='double',
    ...
    def skeleton(self):
        spine_chain = gnChain.Chain(transform_list=self.spline_joints,
                                    prefix=self.side,
                                    suffix='JNT',
                                    name=self.part)
