...
class Foot(gnModule.RigModule):
    ...
    def control_rig(self):
        # create controls
        attr_util = gnAttr.Attribute(add=False)

        # main and secondary IK controls
        self.main_ctrl = gnCtrl.Control(parent=self.control_grp,
                                        shape='cube',
                                        prefix=self.side,
                                        suffix='CTRL',
                                        name=self.part + '_01',
        ...
        self.second_ctrl = gnCtrl.Control(parent=self.main_ctrl.ctrl,
                                          shape='cube',
                                          prefix=self.side,
                                          suffix='CTRL',
                                          name=self.part + '_02',
        ...
        self.toe_piv = gnCtrl.Control(parent=self.second_ctrl.ctrl,
                                      shape='cube',
                                      prefix=self.side,
                                      suffix='CTRL',
        ...
        self.heel_piv = gnCtrl.Control(parent=self.toe_piv.ctrl,
                                       shape='cube',
                                       prefix=self.side,
                                       suffix='CTRL',
                                       ...
                                       ctrl_scale=self.ctrl_scale * 0.2)
        self.in_piv = gnCtrl.Control(parent=self.heel_piv.ctrl,
                                     shape='cube',
                                     prefix=self.side,
                                     suffix='CTRL',
                                     ...
                                     ctrl_scale=self.ctrl_scale * 0.2)
        self.out_piv = gnCtrl.Control(parent=self.in_piv.ctrl,
                                      shape='cube',
                                      prefix=self.side,
                                      suffix='CTRL',
        ...
        self.ball_ctrl = gnCtrl.Control(parent=self.out_piv.ctrl,
        ...
        # control for ankle
        self.ankle_ctrl = gnCtrl.Control(parent=self.ball_ctrl.ctrl,
                                         shape='sphere',
                                         prefix=self.side,
                                         suffix='CTRL',
        ...
        self.toe_ctrl = gnCtrl.Control(parent=self.out_piv.ctrl,
    ...
    def output_rig(self):
        ...
        # build toe chain
        toe_chain = gnChain.Chain(transform_list=[self.guide_list[1],
                                                  self.guide_list[2]],
                                  prefix=self.side,
                                  suffix='toe_JNT',
                                  name=self.part)
        ...
        ik_chain = gnChain.Chain(transform_list=[sc.ik_joints[0],
                                                 toe_chain.joints[0],
                                                 toe_chain.joints[1]],
                                 prefix=self.side,
                                 suffix='ik_JNT',
        ...
        # create blend chain
        self.blend_chain = gnChain.Chain(transform_list=fk.fk_joints,
                                         prefix=self.side,
                                         suffix='switch_JNT',
                                         name=self.part)
    ...
    def add_foot_attributes(self):
        gnAttr.Attribute(node=self.main_ctrl.ctrl,
        ...
        roll = gnAttr.Attribute(node=self.main_ctrl.ctrl, type='double',
                                value=0, keyable=True, name='roll')
        roll_max = gnAttr.Attribute(node=self.main_ctrl.ctrl, type='double',
                                value=30, min=0, keyable=True, name='rollMax')
        toe_roll = gnAttr.Attribute(node=self.main_ctrl.ctrl, type='double',
                                value=0, keyable=True, name='toeRoll')
        heel_roll = gnAttr.Attribute(node=self.main_ctrl.ctrl, type='double',
                                value=0, keyable=True, name='heelRoll')
        bank = gnAttr.Attribute(node=self.main_ctrl.ctrl, type='double',
    ...
    def skeleton(self):
        foot_chain = gnChain.Chain(transform_list=self.blend_chain.joints,
                                   prefix=self.side,
                                   suffix='JNT',
                                   name=self.part)
