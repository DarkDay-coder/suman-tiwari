...
class BipedLimb(gnModule.RigModule, gnIk.Ik, gnFk.Fk):
    def __init__(self,
                 side=None,
                 part=None,
                 guide_list=None,
    ...
    def output_rig(self):
        ...
        if self.create_ik and self.create_fk:
            blend_chain = gnChain.Chain(transform_list=self.src_joints,
                                        prefix=self.side,
                                        suffix='switch_JNT',
                                        name=self.part)
    ...
    def skeleton(self):
        limb_chain = gnChain.Chain(transform_list=self.src_joints,
                                   prefix=self.side,
                                   suffix='JNT',
                                   name=self.part)
        if self.create_fk:
