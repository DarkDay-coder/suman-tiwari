...
import grig.build.guide as gnGuide
...
class Spline:
    ...
    def build_spline_controls(self):
        self.attr_util = gnAttr.Attribute(add=False)

        self.crv_ctrls = []
        # create controls
        self.base_ctrl = gnCtrl.Control(parent=None,
                                        shape='cube',
                                        prefix=self.side,
                                        suffix='CTRL',
        ...
        self.tip_ctrl = gnCtrl.Control(parent=None,
                                       shape='cube',
                                       prefix=self.side,
                                       suffix='CTRL',
                                       name=self.part + '_tip',
        ...
        if self.local_ctrl:
            self.base_local = gnCtrl.Control(parent=self.base_ctrl.ctrl,
                                             shape='arrowFourWay',
                                             prefix=self.side,
                                             suffix='CTRL',
            ...
            self.tip_local = gnCtrl.Control(parent=self.tip_ctrl.ctrl,
                                            shape='arrowFourWay',
                                            prefix=self.side,
                                            suffix='CTRL',
        ...
        if self.mid_ctrl:
            pos = gnXform.find_position_on_curve(self.crv, 0.5)
            self.mid_ctrl = gnCtrl.Control(parent=None,
                                           shape='circle',
                                           prefix=self.side,
                                           suffix='CTRL',
                                           name=self.part + '_mid',
    ...
    def build_spline_chain(self, scale_attr=None):
        if not scale_attr:
            scale_attr = gnAttr.Attribute(node=self.base_ctrl.ctrl,
                                          type='double', value=1, keyable=True,
                                          name='globalScale')

        # build spline chain
        self.spline_chain = gnChain.Chain(prefix=self.side,
                                          suffix='driver_JNT',
                                          name=self.part)
        ...
        if self.stretchy:
            stretch = gnAttr.Attribute(node=self.tip_ctrl.ctrl,
                                       type='double', value=1,
            ...
            for i, jnt in enumerate(self.spline_joints):
                ...
                if par:
                    ...
                    gnChain.stretch_segment(jnt=par, start=par_loc, end=loc,
        ...
        if self.fk_offset:
            ...
            for jnt in self.spline_joints:
                ...
                fk_ctrl = gnCtrl.Control(parent=ctrl_par,
